...
with open('filtered_token_dict.json', 'r') as json_file:
    ...
    if query:
        ...
        for token in stemmed_tokens:
            if token in token_dict:
                current_documents = {doc_id for doc_id, _ in token_dict[token]}
        ...
        if matched_documents:
            for document_id in matched_documents:
                ...
                for token in stemmed_tokens:
                    if token in token_dict:
                        frequency = next((freq for doc_id, freq in token_dict[token] if doc_id == document_id), 0)
